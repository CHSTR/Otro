class LinksController < ApplicationController
  ...
  def inicio
    @interes = Intere.where("tipo like ?", "Departamento").order('id DESC').paginate(page: params[:page], per_page: 7)
    @casillas = Casilla.where("ubicacion = 'links'").order("id ASC")
  end
  ...
  def eliminar
    ...
    respond_to do |format|
      format.html {redirect_to links_url, notice:'fue eliminado'}
    end
  end
  ...
  def eliminarc
    ...
    @casillas.destroy
    respond_to do |format|
      format.html {redirect_to links_url, notice:'Casilla Eliminada.'}
    end  
  end
  ...
  def opcion1
    @casillas = Casilla.where("ubicacion = 'links'").order("id ASC")
    @opciones = Opcion.where("lugar = 'opcion1_links'")
  end

  def opcion2
    @casillas = Casilla.where("ubicacion = 'links'").order("id ASC")
    @opciones = Opcion.where("lugar = 'opcion2_links'")
  end

  def opcion3
    @casillas = Casilla.where("ubicacion = 'links'").order("id ASC")
    @opciones = Opcion.where("lugar = 'opcion3_links'")
  end
